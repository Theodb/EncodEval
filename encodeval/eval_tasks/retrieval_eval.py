...
class RetrievalEval(AbstractEval):
    ...
    def train(self) -> None:
        ...
        if not self.tr_args.do_predict:
            ...
            # Copy custom modeling files if they exist in the original model directory
            import os
            import shutil
            original_model_path = self.config.model_kwargs.get("model_name_or_path", "")
            
            # List of potential custom modeling files
            custom_files = ["modeling_eurobert.py", "modeling_qwen.py", "modeling_qwen3.py"]
            
            for filename in custom_files:
                modeling_file = os.path.join(original_model_path, filename)
                if os.path.exists(modeling_file):
                    dest_file = os.path.join(self.tr_args.output_dir, filename)
                    shutil.copy2(modeling_file, dest_file)
    ...
    def evaluate(self, split: str) -> Dict[str, List]:
        ...
        if eval_dataset is None:
            print(f"WARNING: No {split} dataset available for evaluation")
            return {
                "average": {"ndcg_at_10": 0.0},
                "per_instance": {"ndcg_at_10": []}
            }
