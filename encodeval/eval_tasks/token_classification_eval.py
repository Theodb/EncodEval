...
class TokenClassificationEval(AbstractEval):
    ...
    def evaluate(self, split) -> Dict[str, Dict[str, Union[float, List[float]]]]:
        ...
        # Compute metrics
        average_metrics = self.compute_average_metrics(predictions_token, labels_token, subsets)
    ...
    def compute_average_metrics(self, predictions_token: List[List[int]], 
                               labels_token: List[List[int]], 
                               subsets: List[str] = None) -> Dict[str, float]:
        """
        Compute average F1, precision, and recall scores.
        
        Args:
            predictions_token: Token-level predictions
            labels_token: Token-level labels
            subsets: Language/subset labels for each instance
        ...
        for preds, labs in zip(predictions_token, labels_token):
        ...
        # If we have subsets, compute per-subset metrics
        subset_metrics = {}
        if subsets is not None:
            ...
            for subset, indices in subset_to_indices.items():
                subset_preds = []
                subset_labels = []
                for idx in indices:
                    subset_preds.extend(predictions_token[idx])
                    subset_labels.extend(labels_token[idx])
                
                if subset_preds:
                    subset_f1 = f1_score(subset_labels, subset_preds, average='weighted', zero_division=0)
                    subset_metrics[subset] = subset_f1
        
        return {
            "f1_score": float(f1),
            "precision": float(precision),
            "recall": float(recall),
            **subset_metrics  # Add per-subset F1 scores
        }
