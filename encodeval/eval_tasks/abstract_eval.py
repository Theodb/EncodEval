...
class EvalConfig:
    ...
    Attributes:
        ...
        languages: Optional list of language codes to filter datasets (e.g., ["en"]).
    """
    ...
    def __post_init__(self):
        ...
        if self.languages is not None:
            from encodeval.apply_language_filter import apply_language_filter
            apply_language_filter(self.languages)
        ...
        if train_batch_size is not None:
            gradient_accumulation_steps = (
                train_batch_size / (max(self.tr_args.n_gpu, 1) * self.tr_args.per_device_train_batch_size)
            )
        ...
        # Build output_subdir path components
        subdir_parts = [self.task_type, self.dataset_name]
        if ft_model_config_dir is not None:
            subdir_parts.append(ft_model_config_dir.replace('/', '_'))
        if output_subdir:  # Only add if not empty
            subdir_parts.append(output_subdir)
        output_subdir = "/".join(subdir_parts)
